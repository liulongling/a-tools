...
<#list table.fields as field>
    ...
    <#if field.array>
    public ${field.type} ${field.name} = new ArrayList<>();
    <#else>
    <#if field.defaultValue??>
    public ${field.type} ${field.name} = ${field.defaultValue};
    <#else>
    ...
    public static List<${table.entityName}> getList(List<Integer> ids) {
        return ids.stream().map(${table.entityName}::get).collect(toList());
    }
